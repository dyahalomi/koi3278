...
def loadisos():
    ...
    from inputs import (isodir, inds, magname, wdfiles, wdinds)
    import warnings
...
def logprior(p, isobundle):
    ...
    if (period < 0. or e < 0. or e >= 1. or a < 0. or u10 + u11 >= 1 or
            u20 + u21 >= 1 or M2 < 0. or M2init < 0. or M1 < 0.):
        return -np.inf
...
def loglikeli(p, t, f, ferr, cuts, crowding, isobundle,  minimize=False,
    ...
    #get rid of fferr = inf data points in likelihood function
    #set np.sqrt(2. * np.pi * (fferr_likelihood**2.)) = 1
    ...
    M2chisq = (M2 - predM2)**2./(0.1 * predM2)**2.
    totchisq += M2chisq
    #compute total likelihood
    #see Eastman et al., 2013 equation 2
    #see Christiansen et al., 2017 eq 1 (sec 3.2)
    loglikelihood += - (np.log(np.sqrt(2 * np.pi * ((0.1 * predM2) ** 2.))) + (0.5 * M2chisq)) 
